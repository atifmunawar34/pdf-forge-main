        ...
        {!result ? (
          <div className={`${isFormsStudio ? 'flex-1 min-h-0 flex flex-col' : 'space-y-6'}`}>
            ...
            {tool?.id === 'edit' && (
              <>
                {!isEditingPdf && (
                  <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 max-w-lg mx-auto space-y-6 shadow-sm">
                    <div className="text-center space-y-2">
                      <div className="w-12 h-12 rounded-2xl bg-rose-50 text-rose-600 flex items-center justify-center mx-auto shadow-xs">
                        <Edit3 className="w-6 h-6" />
                      </div>
                      <h3 className="text-xl font-black text-slate-900">Edit PDF Text</h3>
                      <p className="text-xs text-slate-500">
                        Add, edit, and format text directly on your PDF using our Canvas 2D editor.
                      </p>
                    </div>
                    ...
                    <button
                      ...
                      className="w-full py-4 bg-rose-600 hover:bg-rose-700 disabled:opacity-50 text-white font-bold rounded-2xl shadow-md transition flex items-center justify-center space-x-2 cursor-pointer"
                    >
                      {isProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : <><span>Start Editing</span><ArrowRight className="w-4 h-4" /></>}
                    </button>
                  </div>
                )}
